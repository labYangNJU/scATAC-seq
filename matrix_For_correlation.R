...
##readin peak file
KIRP<-read.delim("Cancer_All_peak.bed",header = F)
...
head(tmp)
count_matrix<-matrix(0,ncol = 20,nrow=562709)
count_matrix[,1]<-tmp

index<-c(2:20)
j=2
for (i in index) {
  readname<-paste0("ZY_cluster",i,"_logTF.bam")
  cluster<-scanBam(readname,param=p2,maxMemory=5,package="Rsamtools")
  ##corrected for the Tn5 offset
  strand<-cluster[[1]]$strand
  index<-which(as.character(strand)=="+")
  pos<-cluster[[1]]$pos
  pos[index]<-pos[index]+4
  cluster[[1]]$start[index]<-pos[index]-cluster[[1]]$qwidth[index]
  cluster[[1]]$end[index]<-pos[index]
  index<-which(as.character(strand)=="-")
  pos[index]<-pos[index]-5
  cluster[[1]]$start[index]<-pos[index]
  cluster[[1]]$end[index]<-pos[index]+cluster[[1]]$qwidth[index]
  cluster[[1]]$pos<-pos
  fragement<-cbind(cluster[[1]]$rname,cluster[[1]]$start,cluster[[1]]$end)
  fragement<-as.data.frame(fragement)
  fragement$V1<-paste0("chr",fragement$V1)
  a<-which(fragement$V1=="chr23")
  fragement$V1[a]="chrX"
...
name<-c("cluster1","cluster2","cluster3","cluster4","cluster5","cluster6","cluster7","cluster8","cluster9","cluster10","cluster11",
        "cluster12","cluster13","cluster14","cluster15","cluster16","cluster17","cluster18","cluster19","cluster20")
